...
export const Sidebar: React.FC = () => {
  ...
  const adminMenuItems = user?.role === 'ADMIN' ? [
    ...
    { 
      ...
      submenu: [
        { path: '/admin/members', label: 'All Members' },
        { path: '/admin/members/import', label: 'Import Members' },
        { path: '/admin/members/verify', label: 'Verify Members' },
      ]
    },
    { 
      path: '/admin/reports', 
      ...
      >
        ...
        <div className="p-6 border-b border-gray-200/50">
          <div className="flex items-center justify-between mb-6">
            <div className={`flex items-center space-x-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
              ...
              {!isCollapsed && (
                <div>
                  <h2 className="text-lg font-bold text-gray-800">CGTTI Alumni</h2>
          ...
          {isCollapsed && (
            <div className="flex justify-center mb-4">
              <div className="relative group">
                ...
                <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-white ${user?.role === 'ADMIN' ? 'bg-purple-500' : 'bg-green-500'}`}></div>
          ...
          <ul className="space-y-1">
            {menuItems.map((item, index) => {
              ...
              if (item.type === 'divider') {
                return !isCollapsed ? (
                  <li key={`divider-${item.label}`} className="px-4 py-2">
        ...
        <div className="p-4 border-t border-gray-200/50">
          ...
          {isCollapsed ? (
            <button
              onClick={handleLogout}
              className="w-full p-3 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors flex justify-center group relative"
              aria-label="Logout"
            >
              <LogOut className="w-5 h-5" />
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 px-4 py-2">
                  <Shield className="w-4 h-4 text-green-500" />
                  <span className="text-xs text-gray-500">
                    {user?.role === 'ADMIN' ? 'Admin Mode • ' : ''}Secure • v2.1
                  </span>
                </div>
