...
//         // Add content with proper line breaks and page management
//         pdf.setFontSize(10);
//         pdf.setFont('helvetica', 'normal');
...
//         // Add footer for each member's page
//         const footerY = pageHeight - 15;
//         pdf.setFontSize(8);
//         pdf.setTextColor(107, 114, 128);
//         const footerText = `Generated on: ${new Date().toLocaleDateString()} | Member: ${member.name} | Page ${i + 1} of ${selectedMembers.length}`;
//         pdf.text(footerText, pageWidth / 2, footerY, { align: 'center' });
//       }

//       // Save the combined PDF
//       const fileName = `CGTTI_Letters_Bulk_${new Date().toISOString().split('T')[0]}.pdf`;
//       pdf.save(fileName);
...
//   // Alternative: Export individual PDFs for each member
...
//         pdf.setFontSize(10);
//         pdf.setFont('helvetica', 'bold');
//         pdf.text('Member Details:', margin, yPosition);
//         pdf.setFont('helvetica', 'normal');
...
//           pdf.text(detail, margin, yPosition + 10 + (index * 6));
//         });
...
//         // Save individual PDF
//         const fileName = `CGTTI_Letter_${member.name.replace(/\s+/g, '_')}_${member.membershipNumber}.pdf`;
//         pdf.save(fileName);
...
//       alert(`✅ Successfully generated ${selectedMembers.length} individual PDF files`);
...
//                 >
//                   {exportLoading ? 'Exporting...' : `Export All as Single PDF (${selectedMembers.length})`}
//                 </button>
//                 <button
//                   onClick={exportIndividualPDFs}
//                   disabled={exportLoading || !reason.trim() || (!isCustom && !selectedTemplate) || (isCustom && (!customSubject.trim() || !customContent.trim()))}
//                   className="px-4 py-2 text-sm font-medium text-white bg-purple-600 border border-transparent rounded-md shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
//                 >
//                   {exportLoading ? 'Exporting...' : `Export Individual PDFs (${selectedMembers.length})`}
//                 </button>
//               </>
//             )}
//           </div>
      ...
      <div className="bg-white rounded-lg max-w-6xl w-full max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-6 border-t flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            ...
            <button
              onClick={handleGenerateLetters}
              disabled={loading || (!isCustom && !selectedTemplate) || (isCustom && (!customSubject.trim() || !customContent.trim()))}
              className="px-6 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Generating...' : `Generate & ${emailOption !== 'none' ? 'Email' : 'Save'} ${selectedMembers.length} Letter(s)`}
            </button>
